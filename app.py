    ...
    if menu == "Lançar Despesa":
        ...
        if st.button("💾 Salvar Despesa", type="primary", use_container_width=True):
            if not fornecedor:
                ...
                # --- ROTINA DE LIMPEZA FORÇADA (Agora sobrescrevemos os valores) ---
                st.session_state["val_desp"] = 0.01  # Reseta valor
                st.session_state["obs_desp"] = ""    # Reseta observação
                st.session_state["txt_novo_forn"] = "" # Reseta campo de texto novo forn
                st.session_state["sel_forn"] = ""    # Reseta select (assume que o primeiro item é vazio)
                st.session_state["check_novo_forn"] = False # Desmarca checkbox novo fornecedor
                st.session_state["data_liq_desp"] = datetime.now() # Reseta data para hoje
                
                # Só limpamos a competência SE a memória não estiver ativa
                if not st.session_state.get("check_repetir_comp", False):
                     # Se não for repetir, poderíamos resetar, mas o código já recalcula o mês atual
                     # ao recarregar a página, então apenas deletar essas chaves funciona aqui
